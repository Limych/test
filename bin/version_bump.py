...
VERSION_FILES = ("__init__", "const", "_version")


def _get_git_version():
    """Return version from git."""
    ret = subprocess.check_output(["git", "describe", "--tags"])
    return ret.decode("utf-8").lstrip("v").split("-")[0]
...
def get_package_version(localpath, package):
    ...
    try:
        return_value = Version(return_value)
    except InvalidVersion:
        return_value = Version(_get_git_version())
    return return_value
...
def _write_version(localpath, version, dry_run=False) -> bool:
    ...
    if dry_run:
        _LOGGER.info("%s could was changed", localpath)
    else:
...
def write_version(localpath, version, dry_run=False):
    """Update package constant file with new version."""
    if not _write_version(localpath, version, dry_run):
        for fname in VERSION_FILES:
...
def main() -> int:
    ...
    if arguments.dry_run:
        _LOGGER.warning("Dry Run. No Files Was Changed")

    # pylint: disable=subprocess-run-check
    if arguments.commit and subprocess.run(["git", "diff", "--quiet"]).returncode == 1:
        _LOGGER.error("Cannot use --commit because git is dirty.")
        return 1
    ...
    current = get_package_version(package_path, package)
    ...
    _LOGGER.info("    New version: %s", bumped)
    print(bumped)
...
def test_bump_version():
    ...
    assert bump_version(Version("0.56.0"), "dev") == Version("0.56.1.dev0")
    assert bump_version(Version("0.56.3"), "dev") == Version("0.56.4.dev0")
    assert bump_version(Version("0.56.0b3"), "dev") == Version("0.56.1.dev0")
